...
def get_all_users(response:Response, session: Session = Depends(get_session), admin=Depends(current_admin)):
...
def create_user(user: UserCreateOrUpdate, response: Response, session: Session = Depends(get_session), admin=Depends(current_admin)):
    response.status_code=200
    return create_user_from_service(session, user)

@role.patch("/")
def edit_user_password_by_login(user: UserCreateOrUpdate, response: Response, session: Session = Depends(get_session), admin=Depends(current_admin)):
    response.status_code=200
...
@role.delete("/{login}")
def delete_user_by_login(login:str, session:Session = Depends(get_session), admin=Depends(current_admin)):
...
def get_connection_data_by_login(login: str, session: Session = Depends(get_session), admin=Depends(current_admin)):
