...
def get_config(response:Response, session: Session = Depends(get_session), admin=Depends(current_admin)):
...
def patch_config(response:Response, parameters:list[ParameterAddOrUpdate], session: Session = Depends(get_session), admin=Depends(current_admin)):
...
def post_config(response:Response, parameters:list[ParameterAddOrUpdate], session: Session = Depends(get_session), admin=Depends(current_admin)):
    response.status_code=200
...
def post_site_headers(response:Response, headers:list[SiteAddOrUpdate], session: Session = Depends(get_session), admin=Depends(current_admin)):
    response.status_code=200
...
def patch_config(response:Response, headers:list[SiteAddOrUpdate], session: Session = Depends(get_session), admin=Depends(current_admin)):
    response.status_code=200
    return patch_headers(headers=headers, session=session)

@service.get("/config/raw")
def get_raw_config(response:Response, session: Session = Depends(get_session), admin=Depends(current_admin)):
